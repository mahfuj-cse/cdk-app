...
export class CdkTsRdsStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const instantiate = createLambdaFunction("instantiate", "instantiate");

    instantiate.node.addDependency(rdsInstance);

    // Updated addBookFunction
    const addBookFunction = createLambdaFunction(
      "AddBookFunction",
      "addBook"
    );
    addBookFunction.node.addDependency(rdsInstance);

    // Updated getBooksFunction
    const getBooksFunction = createLambdaFunction(
      "GetBooksFunction",
      "getBooks"
    );
    getBooksFunction.node.addDependency(rdsInstance);
    ...
    // /book endpoint for addBook Lambda
    const bookResource = api.root.addResource("book");
    bookResource.addMethod(
      "POST",
      new apigateway.LambdaIntegration(addBookFunction)
    );
    bookResource.addMethod(
      "GET",
      new apigateway.LambdaIntegration(getBooksFunction)
    );
